class Car:
    """
    Add class description here
    """

    def __init__(self, name, length, location, orientation):
        """
        A constructor for a Car object
        :param name: A string representing the car's name
        :param length: A positive int representing the car's length.
        :param location: A tuple representing the car's head (row, col) location
        :param orientation: One of either 0 (VERTICAL) or 1 (HORIZONTAL)
        """
        # Note that this function is required in your Car implementation.
        # However, is not part of the API for general car types.
        # implement your code and erase the "pass"
        self.__name = name
        self.__length = length
        self.__location = location
        self.__orientation = orientation

    def car_coordinates(self):
        """
        :return: A list of coordinates the car is in
        """
        # implement your code and erase the "pass"
        if self.__orientation == 0:
            lst1 = [self.__location]
            for j in range(1, self.__length):
                lst1.append((self.__location[0] + j, self.__location[1]))
            return lst1

        if self.__orientation == 1:
            lst2 = [self.__location]
            for i in range(1, self.__length):
                lst2.append((lst2[0][0], self.__location[1] + i))
            return lst2

    def possible_moves(self):
        """
        :return: A dictionary of strings describing possible movements permitted by this car.
        """
        # For this car type, keys are from 'udrl'
        # The keys for vertical cars are 'u' and 'd'.
        # The keys for horizontal cars are 'l' and 'r'.
        # You may choose appropriate strings.
        # implement your code and erase the "pass"
        # The dictionary returned should look something like this:
        # result = {'f': "cause the car to fly and reach the Moon",
        #          'd': "cause the car to dig and reach the core of Earth",
        #          'a': "another unknown action"}
        # A car returning this dictionary supports the commands 'f','d','a'.
        dic_0 = {"u": "you can move up", "d": "you can move down"}
        dic_1 = {"r": "you can move right", "l": "you can move left"}
        if self.__orientation == 0:
            return dic_0
        if self.__orientation == 1:
            return dic_1

    def movement_requirements(self, movekey):
        """ 
        :param movekey: A string representing the key of the required move.
        :return: A list of cell locations which must be empty in order for this move to be legal.
        """
        # For example, a car in locations [(1,2),(2,2)] requires [(3,2)] to
        # be empty in order to move down (with a key 'd').
        # implement your code and erase the "pass"
        lst = []
        if self.__orientation == 0 and movekey == "u":
            lst.append(
                (self.__location[0] - 1, self.__location[1]))
            return lst
        if self.__orientation == 0 and movekey == "d":
            lst.append(
                (self.__location[0] + self.__length, self.__location[1]))
            return lst
        if self.__orientation == 0 and (movekey == "r" or movekey == "d"):
            return lst

        if self.__orientation == 1 and movekey == "r":
            lst.append(
                (self.__location[0], self.__location[1] + self.__length))
            return lst
        if self.__orientation == 1 and movekey == "l":
            lst.append(
                (self.__location[0], self.__location[1] - 1))
            return lst
        if self.__orientation == 1 and movekey == "d" or movekey == "l":
            return lst

    def move(self, movekey):
        """ 
        :param movekey: A string representing the key of the required move.
        :return: True upon success, False otherwise
        """
        # implement your code and erase the "pass"
        if self.__orientation == 0 and (movekey == "u" or movekey == "d"):
            if movekey == "u":
                self.__location = self.movement_requirements(movekey)[0]
                return True
            if movekey == "d":
                self.__location = (self.__location[0] + 1, self.__location[1])
                return True
        if self.__orientation == 1 and (movekey == "l" or movekey == "r"):
            if movekey == "r":
                self.__location = (self.__location[0], self.__location[1] + 1)
                return True
            if movekey == "l":
                self.__location = (self.__location[0], self.__location[1] - 1)
                return True
        return False

    def get_name(self):
        """
        :return: The name of this car.
        """
        # implement your code and erase the "pass"
        return self.__name
